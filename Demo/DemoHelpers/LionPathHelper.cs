...
namespace DemoHelpers
{
    ...
    /// Get path for lion
    /// </summary>
    public class LionPathHelper
    {
        #region DATA
        static string strLion =
"f2cc99\n" +
...
"M 99,265 L 96,284 L 92,299 L 73,339 L 73,333 L 87,300 L 99,265 L 99,265 L 99,265\n";
        ...
        /// </summary>
        static LionPathHelper mInstance = null;

        /// <summary>
        /// Gets the static instance of this class
        /// </summary>
        static LionPathHelper Helper
        {
            get
            {
                if (mInstance == null)
                {
                    ...
                    LionPathHelper tmp = new LionPathHelper();
        ...
        bool isBuildCache = false;

        /// <summary>
        /// Get lion polygon
        /// </summary>
        /// <returns></returns>
        public static double[][] GetLionPolygons()
        {
            if (!Helper.isBuildCache)
            {
                Helper.BuildCache();
            }
            return Helper.cachedLionPolygons;
        }


        public static Color[] GetLionColors()
        {
            if (!Helper.isBuildCache)
            {
                Helper.BuildCache();
        ...
        void BuildCache()
        {
            int capacity = 50;
            int polygonCount = 0;
            int currentPolygonEdgeCount = 0;

            #region construct array with capacity
            cachedLionPolygons = new double[capacity][];
            ...
            int ptr = 0;
            int currentColor = 0;
            while (ptr < strLion.Length)
            {
                if (strLion[ptr] != 'M' && Char.IsLetterOrDigit(strLion[ptr])) //Set color of points
                {
                    #region color parsing
                    int color = ptr;
                    while (strLion[ptr] != '\n') ptr++;
                    string temp = strLion.Substring(color, ptr - color);

                    //Conver hexa to decimal number
                    color = 0;
                    int i = temp.Length - 1;
                    int mu = 0;
                    int j;
                    ...
                    {
                        j = 0;
                        while (j < hexaNumber.Length)
                        {
                            if (temp[i] == hexaNumber[j])
                            {
                                //find a hexa number
                                color += j * (int)Math.Pow(16, mu);
                            }
                            j++;
                        }
                        mu++;
                ...
                else
                {
                    ...
                    double y = 0.0;
                    int index;
                    int currentPolygonCapacity = 50;

                    // \n is end of current path
                    while ((strLion[ptr] != '\n') && (ptr < strLion.Length))
                    {
                        index = ptr; //strLion[index] equal 'M' or 'L'
                        while (ptr < strLion.Length && !(Char.IsDigit(strLion[ptr]))) ptr++;
                        int c = ptr;
                        while (ptr < strLion.Length && (Char.IsDigit(strLion[ptr]))) ptr++;
                        x = Double.Parse(strLion.Substring(c, ptr - c));
                        ptr++;
                        c = ptr;
                        while (ptr < strLion.Length && Char.IsDigit(strLion[ptr])) ptr++;
                        string temp = strLion.Substring(c, ptr - c);
                        y = Double.Parse(strLion.Substring(c, ptr - c));

                        if (strLion[index] == 'M')
                        {
                            if (polygonCount > 0)
                            {
                                FinishPosition(polygonCount - 1, currentPolygonEdgeCount);
                            }
                            polygonCount++;
                            if (polygonCount > capacity)
                            {
                                capacity = ResizeCache(capacity);
                            }

                            ConstructPolygon(polygonCount - 1, currentPolygonCapacity);
                            currentPolygonEdgeCount = 1;
                            SetPosition(polygonCount - 1, currentPolygonEdgeCount - 1, x, y);
                            SetColor(polygonCount - 1, currentColor);
                        }
                        else
                        {
                            // resize array if need
                            if (currentPolygonEdgeCount >= currentPolygonCapacity)
                            {
                                currentPolygonCapacity = ResizePolygonSize(polygonCount - 1, currentPolygonCapacity);
                            }
                            currentPolygonEdgeCount++;
                            SetPosition(polygonCount - 1, currentPolygonEdgeCount - 1, x, y);
                        }

                        while (ptr < strLion.Length && strLion[ptr] != '\n' && !Char.IsLetterOrDigit(strLion[ptr])) ptr++;
                    }
            ...
            #region resize to fit size
            #region lion polygons 1
            tempCachedLionPolygons = new double[polygonCount][];
            Array.Copy(cachedLionPolygons, tempCachedLionPolygons, polygonCount);
            cachedLionPolygons = tempCachedLionPolygons;
            #endregion

            #region lion color 1
            tempCachedLionColors = new Color[polygonCount];
            Array.Copy(cachedLionColors, tempCachedLionColors, polygonCount);
            cachedLionColors = tempCachedLionColors;
            #endregion

            #region lion polygon 2
            tempCachedGdiLionPolygon = new System.Drawing.PointF[polygonCount][];
            Array.Copy(cachedGdiLionPolygon, tempCachedGdiLionPolygon, polygonCount);
        ...
        #region void Set color
        void SetColor(int polygonIndex, int colorData)
        {
        ...
        void SetPosition(int polygonIndex, int currentEdgeIndex, double x, double y)
        {
            //if ((x < 0) || (y < 0))
            //{
            //    string str = "";
            //}
            cachedLionPolygons[polygonIndex][currentEdgeIndex * 2] = x;
            cachedLionPolygons[polygonIndex][currentEdgeIndex * 2 + 1] = y;

            cachedGdiLionPolygon[polygonIndex][currentEdgeIndex] = new System.Drawing.PointF((float)x, (float)y);
        }

        void FinishPosition(int polygonIndex, int polygonEdgeCounter)
        {
            double[] temp = new double[polygonEdgeCounter * 2];
            Array.Copy(cachedLionPolygons[polygonIndex], temp, polygonEdgeCounter * 2);
            cachedLionPolygons[polygonIndex] = temp;

            System.Drawing.PointF[] tempGdi = new System.Drawing.PointF[polygonEdgeCounter];
            Array.Copy(cachedGdiLionPolygon[polygonIndex], tempGdi, polygonEdgeCounter);
            cachedGdiLionPolygon[polygonIndex] = tempGdi;
        }

        void ConstructPolygon(int polygonIndex, int capacity)
        {
            cachedGdiLionPolygon[polygonIndex] = new System.Drawing.PointF[capacity];
            cachedLionPolygons[polygonIndex] = new double[capacity * 2];
        }

        #region resize polygon size

        int ResizePolygonSize(int currentPolygonIndex, int oldCapacity)
        {
            int newCapacity = oldCapacity * 2;
            double[] temp = new double[newCapacity * 2];
            Array.Copy(cachedLionPolygons[currentPolygonIndex], temp, oldCapacity * 2);
            cachedLionPolygons[currentPolygonIndex] = temp;

            System.Drawing.PointF[] tempGdi = new System.Drawing.PointF[newCapacity];
            Array.Copy(cachedGdiLionPolygon[currentPolygonIndex], tempGdi, oldCapacity);
            cachedGdiLionPolygon[currentPolygonIndex] = tempGdi;
        ...
        #region resize cache
        int ResizeCache(int oldCapacity)
        {
            int newCacheSize = oldCapacity * 2;
            #region resize to fit size
            #region lion polygons 1
            tempCachedLionPolygons = new double[newCacheSize][];
            Array.Copy(cachedLionPolygons, tempCachedLionPolygons, oldCapacity);
            cachedLionPolygons = tempCachedLionPolygons;
            #endregion

            #region lion color 1
            tempCachedLionColors = new Color[newCacheSize];
            Array.Copy(cachedLionColors, tempCachedLionColors, oldCapacity);
            cachedLionColors = tempCachedLionColors;
            #endregion

            #region lion polygon 2
            tempCachedGdiLionPolygon = new System.Drawing.PointF[newCacheSize][];
            Array.Copy(cachedGdiLionPolygon, tempCachedGdiLionPolygon, oldCapacity);
            cachedGdiLionPolygon = tempCachedGdiLionPolygon;
            #endregion
